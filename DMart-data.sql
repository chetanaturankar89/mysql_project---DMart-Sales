...
drop table order_product_mapping;
...
-- What query will you write for creating the ‘product_info’ table?
...
-- What query will you write for creating the ‘customer_info’ table?
...
-- What query will you write for creating the ‘branch_details’ table?
...
alter table sales_fact add constraint ord_id_fn
foreign  key (order_id) references order_product_mapping(order_id);


-- What query will you write for adding the foreign key constraint to 
-- the ‘order_product_mapping’ table in order to connect it to the ‘product_info’ table?
...
/*What query will you write for adding the foreign key constraint to the 
‘sales_fact’ table in
 order to connect it to the ‘customer_info’ table? */
 
ALTER TABLE sales_fact
ADD CONSTRAINT customer_id_fn
  FOREIGN KEY (customer_id) REFERENCES customer_info (customer_id);
  
/*What query will you write for adding the foreign key constraint to the ‘sales_fact’
 table in order to connect it to
 the ‘branch_details’ table?*/
...
-- DML Statements Syntax
 ...
 -- Database Manipulation - I
  
 -- inserting records in product_info
 
  insert into product_info
 (product_id,product_name,product_category,list_price,sale_price)
values 
...
('55d7','Coca-Cola (Bottle)','soft drinks-juice and soda',80,80);
 ...
 -- inserting records in order_product_mapping
...
-- inserting data into customer info table 
...
-- inserting data into branch details 

 INSERT INTO branch_details
(store_id,store_name,area,city,state)
values 
...
-- inserting data into branch sales_fact 
...
-- 1.  Find the number of transactions involving male customers?

select * from customer_info ; -- customer_id

select * from sales_fact; -- customer_id

/*As customer_id common in bth table will join these 2 table to
 get number of transactions involving male customers*/
...
-- The number of transactions involving male customers 358 male customers

 -- 2. Find the number of customers who placed an order from more than one store?
 
SELECT 
    COUNT(customer_id)
FROM
    (SELECT 
        customer_id, COUNT(DISTINCT store_id) AS store_count
    FROM
        sales_fact
    GROUP BY customer_id
    HAVING COUNT(DISTINCT store_id) > 1) AS a;
    
-- 3. The city with the highest number of orders along with the number of orders?

SELECT CITY FROM BRANCH_DETAILS;
...
LIMIT 1;
 ...
 /*4.the area that recorded the highest sales in terms of sales amount,
 along with its sales amount?*/
...
-- 5. write  a query to find the state with the highest number of customers?
